...
*/
		...
		cMessage *job = new cMessage("new_hp");
		job->addPar("new_hp");
	    job->par("new_hp").setLongValue(new_W_HP);
	    send(job, "out");
		// sendDirect(job, getParentModule()->getSubmodule("netwrk")->gate("in"));
		delete msg;
	}
	else if(!strcmp(msg->getName(), "mp_avg_delay")) {
		mp_avg_delay = round(msg->par("mp_avg_delay").doubleValue());
		ev << "MP =  " << mp_avg_delay;
	}
	else if (!strcmp(msg->getName(), "flcWeight")) {
	    string queueType = msg->par("delayFor").stringValue();

	    if (queueType == "hp") {
	        hp_weight = msg->par("flcWeight").longValue();
            EV << "FLC HP Weight = " << hp_weight << endl;
	    }
	    else if (queueType == "mp") {
            mp_weight = msg->par("flcWeight").longValue();
            EV << "FLC MP Weight = " << mp_weight << endl;
	    }
	}
}
